...
void upf_pfcp_state_associated(ogs_fsm_t *s, upf_event_t *e)
{
    ...
    case UPF_EVT_N4_MESSAGE:
        ...
        case OGS_PFCP_SESSION_MODIFICATION_REQUEST_TYPE:  
            ...
            // 如果检测到 Remove 请求，则清除 TC 规则并退出该分支
            if (remove_requested)
            {
                const char *interface = "ogstun";
                char tc_cmd[512];

                ogs_info("Detected remove IE in PFCP message, clearing tc rules...");

                // 清除 ogstun 上的 qdisc 和 ingress
                snprintf(tc_cmd, sizeof(tc_cmd), "sudo tc qdisc del dev %s root 2>/dev/null", interface);
                system(tc_cmd);
                snprintf(tc_cmd, sizeof(tc_cmd), "sudo tc qdisc del dev %s ingress 2>/dev/null", interface);
                system(tc_cmd);
                // 清除 ifb0 上的 qdisc
                snprintf(tc_cmd, sizeof(tc_cmd), "sudo tc qdisc del dev ifb0 root 2>/dev/null");
                system(tc_cmd);

                ogs_info("✅ All tc rules have been cleared on interface %s", interface);
                // 2) 继续处理 PFCP 请求 -> 产生响应
            ...
            */
            ...
            if (strlen(ue_ip) > 0 && strlen(server_ip) > 0 && mbr_ul > 0)
            {
                ...
                uint32_t rate_mbit = (uint32_t)(mbr_ul / 1000000);
                char rate_str[32] = {0};
                snprintf(rate_str, sizeof(rate_str), "%umbit", rate_mbit);
                ogs_info("Using rate: %s", rate_str);

                // 加载 IFB 模块并启用 ifb0
                snprintf(tc_cmd, sizeof(tc_cmd), "sudo modprobe ifb numifbs=1 && sudo ip link set ifb0 up");
                system(tc_cmd);
                ...
                snprintf(tc_cmd, sizeof(tc_cmd), "sudo tc qdisc add dev %s root handle 1: htb default 30", interface);
                ogs_info("Executing: %s", tc_cmd);
                if (system(tc_cmd) != 0){
                    ogs_error("Failed to add downlink qdisc on %s", interface);
                }
                snprintf(tc_cmd, sizeof(tc_cmd), "sudo tc class add dev %s parent 1: classid 1:10 htb rate %s ceil %s", interface, rate_str, rate_str);
                ogs_info("Executing: %s", tc_cmd);
                if (system(tc_cmd) != 0){
                    ogs_error("Failed to add downlink class on %s", interface);
                }
                snprintf(tc_cmd, sizeof(tc_cmd),
                         "sudo tc filter add dev %s protocol ip parent 1: prio 1 u32 match ip dst %s/32 match ip src %s/32 flowid 1:10",
                         interface, ue_ip, server_ip);
                ogs_info("Executing: %s", tc_cmd);
                if (system(tc_cmd) != 0){
                    ogs_error("Failed to add downlink filter for UE IP %s and server IP %s on %s", ue_ip, server_ip, interface);
                }

                // 🔺 上行限速设置（UE 发送方向）
                snprintf(tc_cmd, sizeof(tc_cmd), "sudo tc qdisc add dev %s handle ffff: ingress", interface);
                ogs_info("Executing: %s", tc_cmd);
                if (system(tc_cmd) != 0){
                    ogs_error("Failed to add ingress qdisc on %s", interface);
                }
                snprintf(tc_cmd, sizeof(tc_cmd),
                         "sudo tc filter add dev %s ingress protocol ip prio 1 u32 match ip src %s/32 match ip dst %s/32 action mirred egress redirect dev ifb0",
                         interface, ue_ip, server_ip);
                ogs_info("Executing: %s", tc_cmd);
                if (system(tc_cmd) != 0){
                    ogs_error("Failed to add uplink ingress filter for UE IP %s and server IP %s on %s", ue_ip, server_ip, interface);
                }
                snprintf(tc_cmd, sizeof(tc_cmd), "sudo tc qdisc add dev ifb0 root handle 2: htb default 30");
                ogs_info("Executing: %s", tc_cmd);
                if (system(tc_cmd) != 0){
                    ogs_error("Failed to add qdisc on ifb0");
                }
                snprintf(tc_cmd, sizeof(tc_cmd), "sudo tc class add dev ifb0 parent 2: classid 2:10 htb rate %s ceil %s", rate_str, rate_str);
                ogs_info("Executing: %s", tc_cmd);
                if (system(tc_cmd) != 0){
                    ogs_error("Failed to add uplink class on ifb0");
                }
                snprintf(tc_cmd, sizeof(tc_cmd),
                         "sudo tc filter add dev ifb0 protocol ip parent 2: prio 1 u32 match ip src %s/32 match ip dst %s/32 flowid 2:10",
                         ue_ip, server_ip);
                ogs_info("Executing: %s", tc_cmd);
                if (system(tc_cmd) != 0){
                    ogs_error("Failed to add uplink filter for UE IP %s and server IP %s on ifb0", ue_ip, server_ip);
                }

                ogs_info("✅ Applied tc rate limiting on %s for traffic from UE %s to server %s at rate %s", interface, ue_ip, server_ip, rate_str);
            }
            else{
                ogs_error("Failed to extract UE IP, server IP or UL MBR for tc rate limiting");
            }
